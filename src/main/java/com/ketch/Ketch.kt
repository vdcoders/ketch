...
class Ketch private constructor(
    ...
    /**
     * Download the content using multiple chunk URLs
     *
     * @param chunkedUrls List of URLs for chunked download
     * @param path Download path to store the downloaded file
     * @param fileName Name of the file to be downloaded
     * @param tag Optional tag for each download to group the download into category
     * @param metaData Optional metaData set for adding any extra download info
    ...
    fun downloadChunked(
        url : String,
        chunkedUrls: MutableList<String>,
        path: String,
        fileName: String,
        tag: String = "",
        metaData: String = "",
        ...
        val downloadRequest = prepareDownloadRequest(
            url = url, // Empty URL for chunked downloads
            path = path,
            fileName = fileName,
            tag = tag,
            headers = headers,
            metaData = metaData,
            supportPauseResume = false, // Chunked downloads don't support pause/resume
    ...
    /**
     * Download the content synchronously using multiple chunk URLs
     *
     * @param chunkedUrls List of URLs for chunked download
     * @param path Download path to store the downloaded file
     * @param fileName Name of the file to be downloaded
     ...
     * @return Unique Download ID associated with current download
     */
    suspend fun downloadChunkedSync(
        chunkedUrls: MutableList<String>,
        path: String,
        fileName: String,
        tag: String = "",
        ...
        val downloadRequest = mutex.withLock {
            prepareDownloadRequest(
                url = "", // Empty URL for chunked downloads
                path = path,
                fileName = fileName,
                tag = tag,
                headers = headers,
    ...
    fun renameFileDb(id: Int, rename: String) {
        downloadManager.renameDbAsync(id, rename)
    }

    fun moveFileDb(id: Int, rename: String,status: Status) {
        downloadManager.moveDbAsync(id, rename,status)
    }
    fun deleteFileDb(id: Int) {
        downloadManager.deleteDbAsync(id)
    }
    ...
    private fun prepareDownloadRequest(
        ...
        chunkedUrls: MutableList<String>? = null,
        totalBytes: Long,
        imagelink: String
    ): DownloadRequest {
        require(url.isNotEmpty() || !chunkedUrls.isNullOrEmpty()) {
            "Either url or chunkedUrls must be provided"

        }
        require(path.isNotEmpty() && fileName.isNotEmpty()) {
    ...
    suspend fun importFilesToDbOnce(
        dir: File,
        status: Status,
        prefsKey: String,
        acceptFile: (File) -> Boolean,
        tagProvider: (File) -> String
    ) = withContext(Dispatchers.IO) {

        val prefs = context.getSharedPreferences("ketch_import", Context.MODE_PRIVATE)
        if (prefs.getBoolean(prefsKey, false)) return@withContext
        ...
        val dao = DatabaseInstance.getInstance(context).downloadDao()

        val files = dir.listFiles()?.filter { it.isFile && acceptFile(it) } ?: emptyList()

        for (file in files) {
            val path = file.parentFile?.absolutePath ?: continue
            val fileName = file.name

            val existingId = dao.getIdByPathAndFileName(path, fileName)
            if (existingId != null) continue

            var newId = makeStableId(path, fileName)
            while (dao.existsId(newId) != null) {
                newId += 1
                if (newId == Int.MAX_VALUE) newId = 1
            }

            val total = file.length()
            val lastMod = file.lastModified().takeIf { it > 0 } ?: System.currentTimeMillis()
            ...
            val entity = DownloadEntity(
                ...
                eTag = "",
                userAction = UserAction.DEFAULT.toString(),
                metaData = "imported_disk",
                failureReason = "",
                chunkedUrlsJson = "",
                imagelink = ""
            )

            dao.insert(entity) // should be INSERT IGNORE in DAO if possible
        }

        prefs.edit { putBoolean(prefsKey, true) }
    }

    private fun makeStableId(path: String, fileName: String): Int {
        val h = (path + File.separator + fileName).hashCode()
        val id = abs(h)
