...
internal class DownloadManager(
    ...
    suspend fun download(downloadRequest: DownloadRequest) {
        ...
        val inputDataBuilder = Data.Builder()
            .putInt(DownloadConst.KEY_DOWNLOAD_ID, downloadRequest.id)
            .putString(DownloadConst.KEY_NOTIFICATION_CONFIG, notificationConfig.toJson())
        ...
        if (downloadDao.find(downloadRequest.id) != null) {
            ...
            downloadDao.insert(
                DownloadEntity(
                    ...
                    // Add chunked URLs to database for tracking
                    chunkedUrlsJson = WorkUtil.listToJson(downloadRequest.chunkedUrls),
                    totalBytes = downloadRequest.totalBytes,
                    imagelink = downloadRequest.imagelink
                )
            )
        }
        ...
        if (downloadEntity != null) {
            ...
            download(
                DownloadRequest(
                    ...
                    // Add chunked URLs when resuming
                    chunkedUrls = WorkUtil.jsonToList(downloadEntity.chunkedUrlsJson),
                    totalBytes = downloadEntity.totalBytes,
                    imagelink = downloadEntity.imagelink
                )
            )
        }
    }
        ...
        if (downloadEntity != null) {
            ...
            download(
                DownloadRequest(
                    ...
                    headers = WorkUtil.jsonToHashMap(downloadEntity.headersJson),
                    metaData = downloadEntity.metaData,
                    // Add chunked URLs when retrying
                    chunkedUrls = WorkUtil.jsonToList(downloadEntity.chunkedUrlsJson),
                    totalBytes = downloadEntity.totalBytes,
                    imagelink = downloadEntity.imagelink
                )
    ...
    fun renameDbAsync(id: Int, rename: String) {
        scope.launch {
            val downloadEntity = downloadDao.find(id)
            downloadEntity?.fileName = rename
            downloadDao.update(downloadEntity!!)
        }
    }
    fun moveDbAsync(id: Int, rename: String, status: Status) {
        scope.launch {
            val downloadEntity = downloadDao.find(id)
            downloadEntity?.fileName = rename
            downloadEntity?.status = status.toString()
            downloadDao.update(downloadEntity!!)
        }
    }
    fun deleteDbAsync(id: Int) {
        scope.launch {
            downloadDao.remove(id)
        }
    }
