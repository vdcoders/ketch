...
internal interface DownloadDao {
    ...
    @Query("SELECT id FROM downloads WHERE path = :path AND fileName = :fileName LIMIT 1")
    suspend fun getIdByPathAndFileName(path: String, fileName: String): Int?

    @Query("SELECT id FROM downloads WHERE id = :id LIMIT 1")
    suspend fun existsId(id: Int): Int?
